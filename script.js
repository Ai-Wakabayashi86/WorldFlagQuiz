...
function nextQuestion() {
    ...
    answer = currentCountries[
        Math.floor(Math.random() * currentCountries.length)
    ];
    ...
    if (quizMode === "flag") {
        document.getElementById("question").innerHTML =
            `<img src="https://flagcdn.com/w640/${answer.code}.png" alt="${answer.name}の国旗">`;
    } else {
        document.getElementById("question").innerHTML = `
            <div class="trivia">
                <p>この国はどこでしょう？</p>
...
function checkAnswer(choice) {
    ...
    if (choice.name === answer.name) {
        ...
        result.textContent = `❌ 不正解！ 正解は「${answer.name}」です`;
    }
...
function showResult() {
    ...
    document.getElementById("result").innerHTML = `
        <h2>結果発表</h2>
        <p>${maxQuestions}問中 ${score}問正解！</p>
        <button onclick="retryQuiz()">再挑戦</button>
